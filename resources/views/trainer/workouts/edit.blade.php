...
<div class="max-w-4xl">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        ...
        <form action="{{ route('trainer.workouts.update', $workout) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4 mb-6">
                    ...
                    <div>
                        ...
                        >
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('category_id', $workout->category_id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        >
                            @foreach($levels as $level)
                                <option value="{{ $level->id }}" {{ old('level_id', $workout->level_id) == $level->id ? 'selected' : '' }}>
                                    {{ $level->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    @if($workout->image)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $workout->image) }}" alt="{{ $workout->title }}" class="h-32 rounded-lg">
                    </div>
                    @endif
                    <input 
                        type="file" 
            ...
            <div class="mb-6">
                ...
                <div id="exercises-container" class="space-y-4">
                    ...
                    <div class="border border-gray-200 rounded-lg p-4 exercise-row" id="exercise-{{ $index + 1 }}">
                        <div class="flex items-center justify-between mb-3">
                            <h4 class="font-medium text-gray-900">Exercise #{{ $index + 1 }}</h4>
                            <button type="button" onclick="removeExercise({{ $index + 1 }})" class="text-red-600 hover:text-red-800 text-sm">Remove</button>
                        </div>
